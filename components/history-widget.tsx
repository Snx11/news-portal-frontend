    ...
    <Card className="overflow-hidden border-2 border-gray-300">
      <CardContent className="p-0">
        <div className="bg-gray-800 text-white p-2 font-medium text-sm flex justify-between items-center">
          <span>Geçmiş</span>
          {history.length > 11 && (
            <Link href="https://news-portal-backend-4.onrender.com//history" className="text-xs text-blue-300 hover:text-blue-100">
              Tümünü Gör
            </Link>
          )}
        </div>
            ...
            <ul className="space-y-2">
              {history.slice(0, 10).map((item) => (
                <li key={`${item.id}-${item.viewedAt}`} className="border-b pb-2 last:border-0">
                  <Link href={`https://news-portal-backend-4.onrender.com//news/${item.id}`} className="block hover:text-blue-600">
              ...
              {history.length > 11 && (
                <li className="text-center pt-1">
                  <Link href="https://news-portal-backend-4.onrender.com//history" className="text-xs text-blue-600 hover:underline">
